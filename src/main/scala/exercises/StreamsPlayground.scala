...
class Cons[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  /*
    ...
    val prepended = 1 #:: s = new Cons(1, s) // lazy evaluation is preserved
   */
  ...
  def filter(predicate: A => Boolean): MyStream[A] =
    ...
    else tail.filter(predicate) // preserves lazy evaluation, at most head is evaluated
...
object StreamsPlayground extends App {
  ...
  println(startFrom0.flatMap(x => new Cons(x, new Cons(x + 1, EmptyStream))).takeAsList(10))
  println(startFrom0.filter(_ < 10).take(10).toList())
  println(startFrom0.filter(_ < 10).take(10).take(20).toList())
  // println(startFrom0.filter(_ < 10).take(11).toList()) // fails as not possible to take 11 from filtered collection
